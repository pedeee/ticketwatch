...
• Removes past events from URL list automatically
...
# ─── Files & constants ─────────────────────────────────────────────────────
URL_FILE   = "urls.txt"         # default when no arg is given
STATE_FILE = "state.json"

# ⚠️ NEW: allow workflow to override with url_batches/batchN.txt
...
def extract_status(html: str) -> Dict[str, Any]:
    ...
    # <p class="date"> (mobile hero layout)
    if not date_str:
